...
import { METRICS, cohortColor, fmtFit, isInterceptMetric, isSlopeMetric, metricOf, sameConfig, type Config, type Metric } from './common'

const ROWSETS = ['Locked cutoffs', 'One cohort, every cutoff'] as const
const SPECTRA_COLOR: Record<string, string> = { raw: '#1f2933', airspec: '#2171b5', deriv2: '#c026d3' }
...
export function SpectraDumbbell({ calib, selected, onSelect }: { calib: CalibFile; selected: Config; onSelect: (c: Config) => void }) {
  ...
  const rows = useMemo<DRow[]>(() => {
    ...
    for (const [key, rs] of groups) {
      ...
      const label = `${r0.co}${r0.cut !== null ? `-${r0.cut}` : ''}${r0.sel && r0.sel !== 'raw' ? ` · sel ${r0.sel}` : ''}`
  ...
  const all = rows.flatMap((r) => r.pts.map((p) => p.v)).sort(d3.ascending)
  ...
  const ext: [number, number] = rows.length ? [d3.quantile(all, 0.02) ?? 0, d3.quantile(all, 0.98) ?? 1] : [0, 1]
  ...
  return (
    <ChartFrame
      id="spectra-dumbbell"
      title="Spectra-space dumbbell — what baseline correction does to each configuration"
      subtitle="One row per cohort × cutoff × selection space; the dots are the same configuration calibrated on raw, AIRSpec-corrected and second-derivative spectra, joined so the change is the mark. Protocol A, fixed evaluation set, rule k. Click a dot to load that configuration above."
      ...
      controls={
        <>
          ...
          {rowset !== 'Locked cutoffs' && <Select label="cohort" value={cohort} options={cohorts} onChange={setCohort} />}
    ...
    >
      <div ref={wrapRef} className="chart-wrap">
        {rows.length === 0 ? (
          <Empty>No configuration has two spectra spaces at {target}.</Empty>
        ) : (
          <svg width={width} height={innerH + MARGIN.top + MARGIN.bottom} className="animated">
            <g transform={`translate(${labelW},${MARGIN.top})`}>
                ...
                return (
                  <g key={row.key}>
                    <text x={-10} y={cy} dy="0.32em" textAnchor="end" fontSize={rowset === 'Locked cutoffs' ? 11.5 : 10} fill={isSelRow ? INK.text : INK.muted} fontWeight={isSelRow ? 600 : 400} fontFamily={FONT.mono}>
                      {rowset === 'Locked cutoffs' ? row.label : `${row.cfg.cut}${row.cfg.sel && row.cfg.sel !== 'raw' ? ` · sel ${row.cfg.sel}` : ''}`}
                    </text>
                      ...
                      return (
                        <circle
                          ...
                          onMouseEnter={(e) =>
                            tip.show(e, [
                              `${row.label} × ${p.sp}`,
                              `${metric} = ${p.v.toFixed(3)}${pinned ? ' (off the axis)' : ''}`,
                              `k = ${p.r.k} · Deming ${fmtFit(p.r.dm, p.r.db)} · OLS ${fmtFit(p.r.om, p.r.ob)}`,
                              `R² ${p.r.r2?.toFixed(3) ?? '—'} · held-out TOR R² ${p.r.ho?.toFixed(3) ?? '—'}`,
              ...
              <XAxis scale={x} y={innerH} label={metric} />
            </g>
          </svg>
        )}
        <Legend
          items={[
            ...Object.keys(SPECTRA_COLOR).map((sp) => ({ label: calib.spectra[sp] ?? sp, color: spectraColor(sp) })),
            ...(rowset === 'Locked cutoffs' ? cohorts.filter((c) => rows.some((r) => r.cfg.co === c)).map((c) => ({ label: calib.cohorts[c] ?? c, color: cohortColor(c), shape: 'line' as const })) : []),
          ]}
          note={isSlopeMetric(metric) ? `shaded = slope box ${calib.slope_box[0]}–${calib.slope_box[1]}` : metric === 'held-out TOR R²' ? `dashed = floor ${calib.heldout_floor}` : 'the joining line is the cohort colour; the dots are the spectra space'}
        />
