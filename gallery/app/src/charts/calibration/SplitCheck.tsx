 ...
 * distance between points is the whole story.
 */
export function SplitCheck({ run, runs }: { run: CalibRun; runs: CalibRunsFile }) {
  ...
  const slopeOf = (r: (typeof rows)[number]) => (est === 'Deming' ? r.m!.deming_slope : r.m!.ols_slope) as number
  const icOf = (r: (typeof rows)[number]) => (est === 'Deming' ? r.m!.deming_intercept : r.m!.ols_intercept) as number

  const innerW = Math.max(240, Math.min(620, width) - MARGIN.left - MARGIN.right)
  ...
  return (
    <ChartFrame
      ...
      subtitle="One fitted model, scored on the early and late halves of the evaluation set and on the odd and even (interleaved) halves, all with the same n. The axes follow the points: the distance between them is the finding. Dotted lines mark slope 1 and intercept 0 when they are in range."
      provenance="calibration_explorer /api/run split_check · Target readout tab"
      controls={<Segmented label="estimator" value={est} options={ESTIMATORS} onChange={setEst} />}
    >
      <div ref={wrapRef} className="chart-wrap">
        {rows.length < 2 ? (
          <Empty>Too few evaluation filters to split into equal halves.</Empty>
        ) : (
          <svg width={Math.min(620, width)} height={height}>
            <g transform={`translate(${MARGIN.left},${MARGIN.top})`}>
              <YAxis scale={y} x={0} label="intercept (µg/m³)" gridWidth={innerW} tickCount={5} />
              <XAxis scale={x} y={innerH} label={`${est} slope`} tickCount={6} />
              ...
              {rows.map((r, i) => (
                <g key={r.split} onMouseEnter={(e) => tip.show(e, [`${SPLIT_LABEL[r.split] ?? r.split} (n=${r.n})`, `slope ${fmt(slopeOf(r), 3)} · intercept ${fmt(icOf(r), 3)} µg/m³`, `R² ${fmt(r.m!.R2 ?? null, 3)}`])} onMouseLeave={tip.hide}>
                  {r.split === 'all'
