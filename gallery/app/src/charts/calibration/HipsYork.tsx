...
export function HipsYork({ runs }: { runs: CalibRunsFile }) {
  ...
  const vals = rows.flatMap((r) => VARIANTS.map((v) => r.fits![v]).filter((f): f is YorkFit => !!f && !('error' in f)).flatMap((f) => [f[what] - f[`${what}_se`], f[what] + f[`${what}_se`]]))
  ...
  return (
    <ChartFrame
      ...
      subtitle={`Weighted York fits of predicted EC against HIPS Fabs at every SPARTAN site under three blank-line conventions, ± 1 SE. Calibration: ${runs.hips.config ?? 'see explorer'}. A site whose intercept is the same under all three carries the offset in the aerosol, not in the transmittance processing.`}
    ...
    >
      <div ref={wrapRef} className="chart-wrap">
            ...
            <g transform={`translate(${LEFT},${MARGIN.top})`}>
              ...
              {rows.map((r) =>
                VARIANTS.map((v, k) => {
                  ...
                  const se = f[`${what}_se`]
                  return (
                    <g key={v} onMouseEnter={(e) => tip.show(e, [`${runs.targets[r.site]?.site ?? r.site} · ${VARIANT_LABEL[v]}`, `${what} ${fmt(val, 3)} ± ${fmt(se, 3)}`, `slope ${fmt(f.slope, 3)} · intercept ${fmt(f.intercept, 3)} · κ ${fmt(f.kappa, 2)}`, `${r.n_matched} of ${r.n} filters matched · ${((r.frac_below_blank_r1 ?? 0) * 100).toFixed(0)} % below the blank R1 range`])} onMouseLeave={tip.hide}>
        ...
        <Legend items={VARIANTS.map((v) => ({ label: VARIANT_LABEL[v], color: VARIANT_COLOR[v] }))} note="bars = ± 1 SE" />
        {runs.hips.blanks && (
          <table className="placement" style={{ maxWidth: 720 }}>
            <thead><tr><th>blank line</th><th>n blanks</th><th>rms lin / quad</th><th>R1 range</th><th>τ₀ mean ± sd</th></tr></thead>
            <tbody>
              {Object.entries(runs.hips.blanks).map(([id, L]) => (
                <tr key={id}>
                  <td className="f">{id}</td>
