...
Historical record of repository consolidation on `main`, found by auditing
`src/`, `research/`, and `notebooks/`. Entries are retained at full technical
detail because they document defects, measurements, rejected audit claims, and
the reasoning behind structural decisions.

This file is chronological and therefore contains superseded backlog language.
Later verdicts control. In particular, merging `src/analysis/quality` with
`src/data/qc` was **declined, not deferred** after measurement showed only two
public-name collisions across 5,660 LOC; do not treat earlier “merge still open”
language as an active item. Current work is listed only in
[`open-items.md`](open-items.md). Standing plotting data moved to
[`plot-taxonomy.md`](plot-taxonomy.md), and import/migration guidance lives in
...
### Quality stacks — all four bugs fixed; historical merge proposal later declined
