...
slide(
    ...
    say=("The full story in one talk, in five acts: what the problem is and what a "
         ...
         "slide 10, the instrument verdict at slide 16, the new cross-site "
         "results at slide 21."),
...
slide(
    "Where the week's asks stand: everything testable is closed",
    fig=F("f_status.png"),
    say=("Score-card first. Corrected selection with corrected calibration: done, "
         "and it found a better cohort than our locked 800. Lot-251-only "
         "evaluation: built. The lot-248 mystery: solved. Bishoftu spectra: pulled "
         "myself from the SPARTAN database — plus Beijing, Delhi and Pasadena "
         "while I was in there. And the lot-versus-aerosol question now has a "
         "quantitative answer."),
    notes=("ETBI pull via Networks_1_0 / SqlClient on the VPN machine; the same "
           "pipeline turns any SPARTAN site into an explorer target. Adama "
           "two-pager: figure drafted (slide 28), text this week."))
...
slide(
    "What a raw spectrum mostly is: ~90% Teflon background at the CH band — and Addis rides a higher one",
...
slide(
    "Raw spectra, Option A: every cohort shares a materially negative intercept — selection alone does not fix raw",
...
slide(
    "Why it works: raw analog similarity was matching Teflon — 4 of 477 filters survive the corrected re-selection",
...
slide(
    "Cohort size is a real lever — and the dense sweep found a basin below the locked 800",
...
slide(
    "Four more cities' spectra, pulled and evaluated — and a lesson about baselines on the way",
...
slide(
    "Everything in this talk is a click: the calibration explorer",
    lines=["Sites tab — five-city evaluation with the extrapolation flag, one click",
           ...
           "Analogs / Selection — every selection recipe inspectable per filter",
           "New: config sidebar; validation guardrails (Q-residual + score distance) on every run"],
    say=("Rather than closing on slides — the tool. Every number today "
         "regenerates from it, and any variant this room proposes, we can run "
         "before the meeting ends. [Live: Sites tab on the winner; HIPS tab "
         "blank-line ladder; if Satoshi proposes a variant, run it now.]"),
    notes=("Explorer on :5058 under anaconda python, cache warm. Fallback if "
           "the demo dies: every figure rebuilds from build_figures/build_deck "
           "in the deliverable folders."))
...
slide(
    ...
    lines=["Per target filter: k most-similar library spectra → small PLS (novel in the FTIR-aerosol literature)",
           ...
           "Addis 4.5x — collapses: neighbors are the library's edge; local ≠ in-domain",
