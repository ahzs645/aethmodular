...
Negative changes mean lower MAE with ID-11-only training. Quarter-specific losses remain visible. The [full common-support ledger](id11_training_blocks.csv) records training/test IDs, counts, ranges and reasons. The [paired predictions](id11_paired_predictions.parquet) preserve original measurement links. All-earlier median/OLS predictions were reconciled to the previous predictions on the common filters.

## Evidence packages

[The extended upstream packet](upstream_questions_v2_draft.md) is **drafted, not sent**. It preserves the original CHTS-0658 result-role question and exact source rows, and adds definitions/applicability of IDs 11/17, model/reference-target mappings, true analytical-batch metadata and original training/evaluation membership. No result is labeled corrected; ChemSpec 217/218 are not treated as prediction versions. Holding filters out downstream does not establish upstream FTIR independence.

[USPA-0257’s candidate evidence package](USPA-0257_evidence_package.md) now includes source-linked collection bounds, the retained session-46 record crosswalk, per-record non-destructive status flags, source definitions and exact remaining questions. All 1,440 rows report status 131648 and trigger none of the limited existing local status checks. This is a candidate screen, not an approved quality decision. Manual-time alignment, active collection and the session-specific observation/correction and quantity-scaling trace remain unresolved. The nominal manufacturer IR BCc unit is identified, but no reviewed eBC mean or absorption comparison is produced. No database search was repeated.

## Reproduction

Run `uv run aeth doctor`, then `uv run python research/ftir_hips_chem/workflows/analyze_filter_proportionality.py`. Add `--notebook` to generate the active notebook and execute its archived copy. Frozen source hashes, specification hash and output/code hashes are in [prefit freeze](prefit_freeze.json) and [manifest](manifest.json).

[All held-out predictions](proportionality_predictions.parquet) include the paired absolute-error differences. [Block comparisons](proportionality_blocks.csv) include training/test IDs, date/EC ranges, nonpositive counts and original split fingerprints. [Both weighting schemes](proportionality_summary.csv) and [all ID-11 sensitivity results](id11_training_summary.csv) are machine-readable.

## Figures

![01_paired_proportionality](/Users/ahmadjalil/github/aethmodular/research/ftir_hips_chem/output/plots/filter_proportionality/01_paired_proportionality.png)

![02_temporal_bias](/Users/ahmadjalil/github/aethmodular/research/ftir_hips_chem/output/plots/filter_proportionality/02_temporal_bias.png)

![03_later_period_models](/Users/ahmadjalil/github/aethmodular/research/ftir_hips_chem/output/plots/filter_proportionality/03_later_period_models.png)

![04_denominator_sensitivity](/Users/ahmadjalil/github/aethmodular/research/ftir_hips_chem/output/plots/filter_proportionality/04_denominator_sensitivity.png)

![05_weighting_and_bias](/Users/ahmadjalil/github/aethmodular/research/ftir_hips_chem/output/plots/filter_proportionality/05_weighting_and_bias.png)

![06_id11_common_test_errors](/Users/ahmadjalil/github/aethmodular/research/ftir_hips_chem/output/plots/filter_proportionality/06_id11_common_test_errors.png)

![07_id11_common_support](/Users/ahmadjalil/github/aethmodular/research/ftir_hips_chem/output/plots/filter_proportionality/07_id11_common_support.png)

![08_quarter_contributions](/Users/ahmadjalil/github/aethmodular/research/ftir_hips_chem/output/plots/filter_proportionality/08_quarter_contributions.png)
